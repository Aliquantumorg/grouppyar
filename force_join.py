...
async def deliver_help_or_join_prompt_pv(context: ContextTypes.DEFAULT_TYPE, user) -> bool:
    """
    تلاش می‌کنه راهنما (یا اگه عضویت اجباری فعاله، پیام عضویت) رو مستقیم به پیوی کاربر بفرسته.
    خروجی True یعنی موفق شد (یعنی کاربر قبلاً بات رو در پیوی استارت کرده).
    خروجی False یعنی نتونست (بات هنوز اجازه‌ی پیام‌دادن به این کاربر رو نداره، باید اول استارت کنه).
    """
    try:
        missing = await _missing_channels(context, user.id)
    except Exception:
    ...
    try:
        if missing:
            ...
            await context.bot.send_message(
                user.id,
                tone.tt(user.id, "force_join_pv", name=name),
                reply_markup=_join_keyboard(missing, user.id, "checkjoinpv"),
            )
        else:
            await context.bot.send_message(
                user.id,
                tone.tt(user.id, "help_intro") + "\n\n" + HELP_TEXT,
                parse_mode="Markdown",
            )
        return True
    except Exception:
...
async def check_join_pv_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(tone.tt(query.message.chat.id, "pv_join_success"))
    # طبق وعده‌ای که به کاربرانِ غیرادمین در گروه داده می‌شه: بعد از عضویت، راهنما همین‌جا ارسال می‌شه
    await context.bot.send_message(
        query.message.chat.id,
        tone.tt(query.message.chat.id, "help_intro") + "\n\n" + HELP_TEXT,
        parse_mode="Markdown",
    )
