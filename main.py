...
async def master_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "راهنما":
        if chat.type in ("group", "supergroup") and not await can_run_admin_command(update, context):
            sent = await force_join.deliver_help_or_join_prompt_pv(context, user)
            if sent:
                await update.message.reply_text(f"{EMOJI['check']} یه پیام توی پیوی برات فرستادم 📩")
            else:
                await update.message.reply_text(
                    f"{EMOJI['warn']} اول باید من رو توی پیوی استارت کنی؛ بعدش خودم راهنما رو برات می‌فرستم."
                )
            return
    ...
    if text == "لغو بازی":
        await games.cancel_game(update, context)
        return
